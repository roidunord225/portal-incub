...
interface ClientTicketDetailPageProps {
  ticket: Ticket;
  user: User; // The client user
...
const ClientTicketDetailPage: React.FC<ClientTicketDetailPageProps> = ({ ticket, user, onAddMessage, onBack }) => {
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() || attachment) {
      ...
      setNewMessage('');
      setAttachment(null);
      const fileInput = document.getElementById('file-upload-client') as HTMLInputElement;
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 h-fit">
          ...
          <div className="space-y-3 text-sm">
            <div>
              ...
              <p className={`w-full p-2 text-sm font-bold rounded-lg text-center ${getStatusColor(ticket.status)}`}>
                {ticket.status}
              </p>
            </div>
            <div>
