            ...
            <ul className="space-y-4">
              {contracts.map(contract => (
                <li key={contract.id} className="flex items-center p-3 bg-gray-50 rounded-lg">
                  <div className="bg-blue-600 p-3 rounded-full mr-4">{getServiceIcon(contract.serviceName)}</div>
        ...
        <div className="space-y-8">
          <DashboardCard title="Support">
            <div className="space-y-3">
              <button onClick={() => onNavigate(View.NewTicket)} className="w-full flex items-center justify-center bg-red-500 text-white font-semibold px-4 py-3 rounded-lg hover:bg-red-600 transition duration-300">
                <i className="fas fa-exclamation-triangle mr-2"></i> Signaler un Incident
              </button>
              <button onClick={() => onNavigate(View.NewTicket)} className="w-full flex items-center justify-center bg-blue-600 text-white font-semibold px-4 py-3 rounded-lg hover:bg-blue-700 transition duration-300">
          ...
          <DashboardCard title="Rapports & Factures">
             <ul className="space-y-3">
                {documents.map(doc => (
                    <li key={doc.id}>
                        <a href={doc.url} className="flex items-center p-3 text-blue-700 hover:bg-blue-50 rounded-lg transition duration-200">
