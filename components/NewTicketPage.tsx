      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-lg space-y-6">
        <div>
            ...
            <div className="flex space-x-4">
                <button type="button" onClick={() => setTicketType('Incident')} className={`w-full p-4 rounded-lg font-bold text-center transition ${ticketType === 'Incident' ? 'bg-incubtek-orange text-white shadow-lg' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                    <i className="fas fa-exclamation-triangle mr-2"></i> Incident
                </button>
                <button type="button" onClick={() => setTicketType('Demande')} className={`w-full p-4 rounded-lg font-bold text-center transition ${ticketType === 'Demande' ? 'bg-incubtek-green text-white shadow-lg' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
        ...
        <div>
            ...
            <input type="file" id="attachment" className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100"/>
        </div>


        <div className="flex justify-between items-center pt-4">
            <button type="button" onClick={onBack} className="text-gray-600 font-semibold hover:text-gray-800">Annuler</button>
            <button type="submit" disabled={loading} className="bg-incubtek-orange text-white font-semibold px-8 py-3 rounded-lg hover:bg-orange-600 transition duration-300 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center">
                {loading ? (
                    <>
